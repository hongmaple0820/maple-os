...
export function WorkflowManager() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowCreate(false); setCreateName(""); await loadWorkflows();
  ...
  const handleExecute = async (workflowId: string) => {
    ...
    try {
      const result = await rpcCall<{ exec_id: string; status: string; error?: string }>("workflow.execute", { workflow_id: workflowId });
      if (result.error) { alert(`执行失败: ${result.error}`); }
      else { alert(`执行成功! exec_id: ${result.exec_id}, 状态: ${result.status}`); }
    } catch (err) { alert(`执行出错: ${(err as Error).message}`); }
    ...
    <div className="flex flex-col h-full">
      <div className="border-b p-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索工作流..." className="w-48" />
          <Button onClick={() => setShowCreate(true)}>新建工作流</Button>
        </div>
      </div>
      {showCreate && (
        <div className="border-b p-4 bg-muted/50 flex items-center gap-2">
          <Input value={createName} onChange={(e) => setCreateName(e.target.value)} placeholder="工作流名称..." className="w-48" />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {filtered.length === 0 && <div className="text-center text-muted-foreground py-8">暂无工作流数据</div>}
        {filtered.map((wf) => (
          <Card key={wf.id} className="hover:shadow-md transition-shadow">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base">{wf.name}</CardTitle>
                <Badge variant={statusVariant[wf.status] ?? "outline"}>{statusLabel[wf.status] ?? wf.status}</Badge>
              </div>
            </CardHeader>
            <CardContent>
              ...
              <div className="flex gap-2 mt-3">
                ...
                <Button size="sm" onClick={() => handleExecute(wf.id)} disabled={executing === wf.id}>{executing === wf.id ? "执行中..." : "运行"}</Button>
