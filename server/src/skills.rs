...
// ── Calculator skill (from PR #103 by LokiLiu + PR #101 by vipera-iso) ──
struct CalculatorSkill;
impl Skill for CalculatorSkill {
    fn id(&self) -> &str { "calculator" }
    fn description(&self) -> &str { "Evaluate arithmetic expressions safely (no eval)" }
    ...
    fn execute(&self, config: &Value) -> anyhow::Result<Value> {
        ...
        if expr.trim().is_empty() { return Ok(serde_json::json!({"error": "expression is empty"})); }
...
fn safe_eval(expr: &str) -> anyhow::Result<f64> {
    ...
    enum Token { Num(f64), Plus, Minus, Star, Slash, Percent, LParen, RParen }
            ...
            match t {
                ...
                op => {
                    while let Some(top) = stack.last() {
                        if top == &Token::LParen || precedence(top) < precedence(op) { break; }
    ...
    fn eval_postfix(postfix: &[Token]) -> anyhow::Result<f64> {
        ...
        for t in postfix {
            match t {
                ...
                Token::Plus => { let b = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; let a = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; stack.push(a + b); }
                Token::Minus => { let b = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; let a = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; stack.push(a - b); }
                Token::Star => { let b = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; let a = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; stack.push(a * b); }
                Token::Slash => { let b = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; if b == 0.0 { return Err(anyhow::anyhow!("division by zero")); } let a = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; stack.push(a / b); }
                Token::Percent => { let b = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; if b == 0.0 { return Err(anyhow::anyhow!("modulo by zero")); } let a = stack.pop().ok_or_else(|| anyhow::anyhow!("malformed"))?; stack.push(a % b); }
